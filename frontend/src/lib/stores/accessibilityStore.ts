...
export type AccessibilityProfile = 'default' | 'dyslexia' | 'adhd' | 'autism' | 'motor' | 'low-vision';
export type FontFamily = 'system' | 'opendyslexic' | 'arial' | 'verdana' | 'times';
export type ColorTheme = 'light' | 'dark' | 'cream' | 'high-contrast' | 'blue-light';
...
const profilePresets: Record<AccessibilityProfile, Partial<AccessibilityState>> = {
	default: {},
	dyslexia: {
...
function loadFromStorage(): AccessibilityState {
	...
	return {
		...
		colorTheme: prefersHighContrast ? 'high-contrast' : (prefersDarkMode ? 'dark' : 'light')
	};
}
...
function createAccessibilityStore() {
	const { subscribe, set, update } = writable<AccessibilityState>(loadFromStorage());
	...
	return {
		...
		updateSetting<K extends keyof AccessibilityState>(key: K, value: AccessibilityState[K]) {
			update(state => {
				const newState = { ...state, [key]: value, profile: 'default' as AccessibilityProfile };
				saveToStorage(newState);
				applyStyles(newState);
				return newState;
			});
...
function applyStyles(state: AccessibilityState) {
	...
	// Color theme classes
	root.classList.remove('theme-light', 'theme-dark', 'theme-cream', 'theme-high-contrast', 'theme-blue-light');
...
export const isReducedMotion = derived(accessibilityStore, $state => $state.reduceMotion);
export const isFocusMode = derived(accessibilityStore, $state => $state.focusModeEnabled);
export const isHighContrast = derived(accessibilityStore, $state => $state.colorTheme === 'high-contrast');
export const isTTSEnabled = derived(accessibilityStore, $state => $state.textToSpeechEnabled);
