...
export function talentToResource(talent: Talent): UnifiedResource {
  ...
  return {
    id: `talent-${talent.id}`,
    type: 'talent',
    name: talent.full_name || `${talent.first_name || ''} ${talent.last_name || ''}`.trim() || talent.email,
    ...
    performance: {
      tasksCompleted: 0, // TODO: get from project assignments
...
export function agentToResource(agent: Agent): UnifiedResource {
  ...
  return {
    ...
    hourlyRate: agent.cost_data?.cost_per_interaction ? agent.cost_data.cost_per_interaction * 10 : 0,
    utilization: agent.status === 'busy' ? 100 : agent.status === 'active' ? 50 : 0,
    performance: {
